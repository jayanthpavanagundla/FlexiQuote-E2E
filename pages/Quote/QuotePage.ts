...
export class QuotePage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        // Quote List Search
        this.quoteSearchInput = page.getByRole('textbox', { name: 'Search' }).first();

        this.quoteNumber = page.locator('span.has-text-weight-bold.mx-1');

        // Vehicle Details
        this.regoInput = page.locator("//div[contains(@class,'autocomplete')]//input[@type='text' and @maxlength='10']");
        ...
        // Color
        this.colorInput = page.locator("//label[contains(.,'Colour')]/following-sibling::p//input");
        // VIN + Engine
        this.vinInput = page.locator('input[name="vin"]');
        this.engineInput = page.locator('.field-body > div:nth-child(2) > .control > .input').first();
        ...
        // Paint fields
        this.paint1 = page.locator('div:nth-child(11) > .field-body > div > .control > .input').first();
        this.paint2 = page.locator('div:nth-child(11) > .field-body > div:nth-child(2) > .control > .input');
        ...
        // Alt Contact
        this.addAltContact = page.getByRole('button', { name: 'Add Alternative Contact add' });
        this.removeAltContact = page.getByRole('button', { name: 'Remove Alternative Contact' });
        this.altFirstName = page.getByRole('textbox', { name: 'First Name' }).nth(1);
        this.altLastName = page.getByRole('textbox', { name: 'Last Name' }).nth(1);

        // Address
        this.addressInput = page.getByRole('textbox', { name: 'Start typing address' });
        this.addressSelect = page.getByRole('button', { name: '󰍎 Test Ridge Trail, Banda' });
    ...
    async editVehicleDetails(data: { rego?: string; vin?: string; color?: string }) {
    ...
    async selectSchedule(data: { scheduleSearch?: string; scheduleOption?: string }) {
        await step(`Select Schedule "${data.scheduleOption ?? ''}"`, async () => {
            await this.scheduleBtn.click();
            if (data.scheduleSearch) await this.scheduleInput.fill(data.scheduleSearch);
            await this.filterBtn.click();
            if (data.scheduleOption) await this.page.getByRole('cell', { name: data.scheduleOption }).click();
        });
    }

    async fillVehicleSpecs(data: { model?: string; transmission?: string; color?: string; vin?: string; engine?: string; year?: string; cyl?: string; cc?: string }) {
        await step('Fill Vehicle Specs', async () => {
            if (data.model) await this.dropdown1.selectOption(data.model);
            if (data.transmission) await this.dropdown2.selectOption(data.transmission);
    ...
    async selectExistingCustomer(firstName: string) {
        await step(`Select Existing Customer "${firstName}"`, async () => {
            ...
            await this.page.getByRole('button', { name: new RegExp(firstName, 'i') }).click();
            ...
            if (primary) {
                await Promise.all([
                    this.page.waitForResponse((r) =>
                        r.url().includes('/UniqApi/v1/insurers/') && r.request().method() === 'GET' && r.status() === 200
                    ),
    ...
    async updateInsurer(secondary: string) {
        await step(`Update Insurer to "${secondary}"`, async () => {
            await this.insurerDropdown.click();
            if (secondary) await this.page.getByText(secondary, { exact: true }).first().click();
        });
    }
