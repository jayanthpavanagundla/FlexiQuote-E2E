...
export class QuoteItemsPage {
    ...
    constructor(page: Page) {
        ...
        // LOGIN
        this.username = page.getByRole('textbox', { name: 'Username New Password Confirm' });
        ...
        this.repairerQuote = page.locator('#main-nav-menu').getByText('Repairer Quote');
        this.searchBox = page.getByRole('textbox', { name: 'Search the system...' });
        this.searchResult = page.getByRole('button', { name: '367FGBICR 27701 Quote To Be' });
        this.editQuote = page.getByRole('link', { name: 'edit_document Quoting' });

        // DELETE BUTTONS
        this.deleteBtn = page.getByRole('button', { name: 'Delete delete' });
        this.deleteMiscBtn = page.getByRole('button', { name: 'Delete Misc Item delete' });
        ...
        this.itemDescription = page.getByRole('textbox', { name: 'Item Description' });
        this.mechItemDescription = page.getByRole('textbox', { name: 'Item Description' });
        ...
        this.subletTag = page.locator('.is-flex.is-clickable.is-align-items-center > .tag');
        ...
        this.labourRows = this.labourSection.locator('.item-row-quote-builder-labour');
        ...
        this.itemRows = page.locator('article, tr, .item-row-quote-builder-parts, .item-row-quote-builder-labour');
        this.rateInputs = page.getByRole('textbox', { name: 'Rate' });
        this.itemDescSpan = page.locator('span.is-quote-item-field-editable[id$="-itemDesc"]');
    ...
    async fillRateAndHours({ rate, hours }: { rate?: string; hours?: string } = {}) {
    ...
    async clickAddItem(category: string | number = 0) {
        const index = typeof category === 'string'
            ? CATEGORY_INDEX[category.toLowerCase()]
            : category;
        await this.page.getByRole('button', { name: '+ Add Item' }).nth(index).click();
    }
    ...
    getItemRow(description: string) {
        return this.page.locator('article, tr, .item-row-quote-builder-parts, .item-row-quote-builder-labour').filter({
            hasText: description,
        }).first();
    }

    async selectCategory(name: string) { await this.getCategoryTab(name).click(); }
    ...
    async fillLabourItem({ description, rate, hours }: { description?: string; rate?: string; hours?: string }) {
    ...
    async fillPartItem({ description, partNo, qty }: { description?: string; partNo?: string; qty?: string }) {
        if (description) await this.fillEditableField(this.itemDescription, description);
    ...
    async fillMechItem({ description, hours }: { description?: string; hours?: string }) {
        if (description) {
            const descLocator = await this.mechItemDescription.isVisible().catch(() => false)
    ...
    async selectMiscItem() { await this.paintMaterial.click(); }

    async fillMiscItem({ description, rate }: { description?: string; rate?: string }) {
        if (description) await this.fillEditableField(this.itemDescription, description);
    ...
    async fillSubletItem({ description, amount, vendor }: { description?: string; amount?: string; vendor?: boolean }) {
        if (description) await this.fillEditableField(this.itemDescription, description);
