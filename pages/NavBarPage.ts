...
export class NavBarPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.repairerQuote = this.dropdownContainer.getByText("Repairer Quote", {
      exact: true,
    });
    this.miscQuote = this.dropdownContainer.getByText("Misc Quote", {
      exact: true,
    });
    this.quoteTemplate = this.dropdownContainer.getByText("Quote Template", {
      exact: true,
    });
    this.bookings = this.dropdownContainer.getByText("Bookings", {
      exact: true,
    });
    this.audaNetTasks = this.dropdownContainer.getByText("AudaNet Tasks", {
      exact: true,
    });
    this.pnetMessages = this.dropdownContainer.getByText("PNET Messages", {
      exact: true,
    });
    this.estimateMessages = this.dropdownContainer.getByText(
      "Estimate Messages",
      { exact: true },
    );
    this.ormMessages = this.dropdownContainer.getByText("ORM Messages", {
      exact: true,
    });
    this.ormBatching = this.dropdownContainer.getByText("ORM Batching", {
      exact: true,
    });
    this.imageUploader = this.dropdownContainer.getByText("Image Uploader", {
      exact: true,
    });
    ...
    this.quickInvoice = this.dropdownContainer.getByText("Quick Invoice", {
      exact: true,
    });
    this.debtorAdjustment = this.dropdownContainer.getByText(
      "Debtor Adjustment",
      { exact: true },
    );
    this.receiptEntry = this.dropdownContainer.getByText("Receipt Entry", {
      exact: true,
    });

    // CREDITOR DROPDOWN
    this.creditorMenu = page.locator("a.navbar-link", { hasText: "Creditor" });
    this.sundryCreditor = this.dropdownContainer.getByText("Sundry Creditor", {
      exact: true,
    });
    this.paymentEntry = this.dropdownContainer.getByText("Payment Entry", {
      exact: true,
    });
    this.paymentEntryTitle = this.page
      .locator("h1.title span")
      .filter({ hasText: "Payment Entry" })
      .first();
    this.purchaseOrder = this.dropdownContainer.getByText("Purchase Order", {
      exact: true,
    });
    this.returnParts = this.dropdownContainer.getByText("Return Parts", {
      exact: true,
    });
    ...
    this.dueInDueOut = this.dropdownContainer.getByText(" Due In & Due Out ", {
      exact: true,
    });
    this.jobInvoiced = this.dropdownContainer.getByText("Job Invoiced", {
      exact: true,
    });
    this.outStandingParts = this.dropdownContainer.getByText(
      "Outstanding Parts",
      { exact: true },
    );
    this.outstandingCredits = this.dropdownContainer.getByText(
      "Outstanding Credits",
      { exact: true },
    );
    this.salesAnalysis = this.dropdownContainer.getByText("Sales Analysis", {
      exact: true,
    });
    this.debtorList = this.dropdownContainer.getByText("Debtor List", {
      exact: true,
    });
    this.receipts = this.dropdownContainer.getByText("Receipts", {
      exact: true,
    });
    this.creditorList = this.dropdownContainer.getByText("Creditor List", {
      exact: true,
    });
    this.paymentList = this.dropdownContainer.getByText("Payment List", {
      exact: true,
    });
    ...
    this.customer = this.dropdownContainer.getByText("Customer", {
      exact: true,
    });
    this.vendor = this.dropdownContainer.getByText("Vendor", { exact: true });
    this.contactProfile = this.dropdownContainer.getByText("Contact Profile", {
      exact: true,
    });
    this.recurringRemarks = this.dropdownContainer.getByText(
      "Recurring Remarks",
      { exact: true },
    );
    this.quickItem = this.dropdownContainer.getByText("Quick Item", {
      exact: true,
    });
    this.item = this.dropdownContainer.getByText("Item", { exact: true });
    this.otherLabour = this.dropdownContainer.getByText("Other labour", {
      exact: true,
    });
    this.vehicle = this.dropdownContainer.getByText("Vehicle", { exact: true });
    this.unscheduledModel = this.dropdownContainer.getByText(
      "Unscheduled Model",
      { exact: true },
    );
    ...
    this.glMapping = this.dropdownContainer.getByText("G/L Mapping", {
      exact: true,
    });
    this.emailSMS = this.dropdownContainer.getByText("Email/SMS Log", {
      exact: true,
    });
