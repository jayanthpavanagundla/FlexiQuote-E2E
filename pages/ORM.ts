...
import { generateInsuranceClaimNo, generateRegistrationNo, getFutureDateTime, } from '../helpers/dataGenerators.js';
...
export class ORM extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.loadMessageButton = this.page.locator('a[data-tooltip="Load Message"]').nth(0);
        this.nextBtn = this.page.getByRole('button', { name: 'Next' });
        this.activeModal = this.page.locator('.modal.is-active');
        this.firstVisibleRow = this.activeModal.locator('table:visible tbody tr').first();
        ...
        this.yesButton = this.page.locator('.modal.is-active button.button.is-primary', { hasText: /^Yes\s*$/ }).first();

        // Filters
        this.searchInput = this.page.locator('input[placeholder="Search..."]');

        // ORM Quote Locator
        this.ormQuoteNo = this.page.locator('table:visible tbody tr').first().locator('td').first().locator('a[href]').first();

        // Header Locators
        this.transmissionDropdown = this.page.locator('div:nth-child(7) > div > .field > .control > .select > select').first();
        this.colourInput = this.page.locator('input[maxlength="20"][aria-autocomplete="list"]').first();
        this.odometerInput = this.page.locator('input[placeholder="e.g. 6"][content*="greater than 0"]');
        this.estimatorInput = this.page.locator('input[placeholder="Quoter"]');
        this.estimateStartDateInput = this.page.locator('input[placeholder="Job Start Date"]').first();
        this.estimateEndDateInput = this.page.locator('input[placeholder="Job End Date"]').first();

        // Quoting Tab Locators
        this.quotingLink = this.page.getByRole('link', { name: 'Quoting' });
        this.vehicleSectionsTab = this.page.locator('li', { hasText: 'Vehicle Sections' });
        this.manualSectionsTab = this.page.locator('li', { hasText: 'Manual Quote' });
        this.templatesTab = this.page.locator('li', { hasText: 'Templates' });

        // Invoice Tab Locators
        this.invoiceLink = this.page.getByRole('link', { name: 'receipt_long Invoice' });
        this.createInvoice = this.page.locator('.tooltip > .button');
        this.saveInvoice =  this.page.locator('a.button[data-tooltip="Save Changes"]');
        this.sendTaxInvoice = this.page.locator('button[data-tooltip="Send Tax Invoice"]');
        this.invoiceCheckBox = this.page.locator('td.has-text-centered.is-content-width .pretty.p-icon');
        this.raiseInvoice = this.page.getByRole('button', { name: /Raise Invoice/i });

        // ORM Tab Locators
        this.ormLink = this.page.getByRole('link', { name: 'message ORM' });
        this.ormDropdown = this.page.getByRole('button', { name: /^ORM$/ });
        this.submitQuote = this.page.locator('a.dropdown-item', { hasText: 'Submit Quote' });
        this.submitTaxInvoice = this.page.locator('a.dropdown-item', { hasText: 'Submit Tax Invoice' });
        this.viewMsgs = this.page.locator('a.dropdown-item', { hasText: 'View Messages' });
        this.angleNextButton = this.page.locator('button[data-tooltip="Next"]:has(i.mdi-chevron-right)');
        this.submitORMButton = this.page.locator('button[data-tooltip="Submit"]');
        this.viewMsgsFirstRow = this.page.locator('.button.is-primary.is-small').first();
        ...
        // Validation Locators
        this.quoteStatusSelect = this.page.locator('tr', { hasText: 'Quote Status' }).locator('select');
    ...
    // RFQ XML Paste
    // ─────────────────────────────────────────────
    async pasteRfqXml(): Promise<{ insuranceClaimNo: string; registrationNo: string }> {
        ...
        await this.page.evaluate((xmlContent: string) => {
            const cmInstance = (document.querySelector('.CodeMirror') as any).CodeMirror;
    ...
    async getXmlContent(): Promise<string> {
        return await step('Get XML content from API', async () => {
            if (!this.capturedXmlContent) {
                throw new Error('No XML content captured. Ensure selectViewMsgFirstRow() was called first.');
            }
    ...
    replaceCPWithA(xml: string): string {
        return xml.replace(/<QuoteType>CP<\/QuoteType>/, '<QuoteType>A</QuoteType>');
    }
    ...
    async getAndModifyQMLQuote(): Promise<string> {
        return await step('Read, modify, and store updated XML (without pasting)', async () => {
    ...
    async pasteModifiedQMLQuote(): Promise<void> {
        await step('Paste stored modified XML into CodeMirror editor', async () => {
            ...
            await this.page.locator('.CodeMirror').waitFor({ state: 'visible', timeout: 10000 });
            ...
            await this.page.waitForFunction((xmlText: string) => {
                const cm = (document.querySelector('.CodeMirror') as any)?.CodeMirror;
                return cm && cm.getValue().trim() === xmlText.trim();
            }, this.modifiedXml, { timeout: 10000 });
        });
    }
    ...
    extractXmlTagValue(xml: string, tagName: string, fallback: string = ''): string {
        const regex = new RegExp(`<${tagName}>([\\s\\S]*?)<\\/${tagName}>`);
        const match = xml.match(regex);
        return match ? match[1].trim() : fallback;
    }

    extractXmlFieldValue(xml: string, tagName: string, fieldName: string, fallback: string = ''): string {
    ...
    replaceXmlPlaceholder(xml: string, placeholder: string, value: string): string {
        return xml.split(`{{${placeholder}}}`).join(this.escapeXmlValue(value));
    }

    modifyQMLPaymentAuthorised(xml: string): string {
        const templatePath = path.resolve(__dirname, '../helpers/payment-authorized.xml');
        ...
        const underwriterName = this.extractXmlTagValue(xml, 'UnderwriterName', 'Insurance Australia Limited');
        const insurerAbn = this.extractXmlFieldValue(xml, 'InsurerDetail', 'ABN', '11000016722');

        const repairerId = this.extractXmlTagValue(xml, 'RepairerID', 'U4073');
        const repairerName = this.extractXmlTagValue(xml, 'RepairerName', 'SKY Smash & Repair');
        const repairerAbn = this.extractXmlFieldValue(xml, 'RepairerDetails', 'ABN', '11223491505');

        const vehicleId = this.extractXmlTagValue(xml, 'VehicleID');
        const insuranceClaimNo = this.extractXmlTagValue(xml, 'InsuranceClaimNo');
        const reinstatementNo = this.extractXmlTagValue(xml, 'ReinstatementNo', '1');
        ...
        const repairerTaxInvoiceNo = this.extractXmlTagValue(xml, 'RepairerTaxInvoiceNo');
        ...
        if (!insuranceClaimNo) throw new Error('InsuranceClaimNo not found in copied XML.');
        if (!quoteNo) throw new Error('QuoteNo not found in copied XML.');
        if (!registrationNo) throw new Error('RegistrationNo not found in copied XML.');
        ...
        if (!repairerTaxInvoiceNo) throw new Error('RepairerTaxInvoiceNo not found in copied XML.');
        if (!payAuthorisedDate) throw new Error('TaxInvoiceDate not found in copied XML.');
        if (!paymentAmount) throw new Error('TotalPayableAmount not found in copied XML.');

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'TIMESTAMP', this.getCurrentQMLTimestamp());
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'MESSAGE_NO', incrementedMessageNo);

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'UNDERWRITER_NAME', underwriterName);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'INSURER_ABN', insurerAbn);

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REPAIRER_ID', repairerId);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REPAIRER_NAME', repairerName);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REPAIRER_ABN', repairerAbn);

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'VEHICLE_ID', vehicleId);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'INSURANCE_CLAIM_NO', insuranceClaimNo);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REINSTATEMENT_NO', reinstatementNo);

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'QUOTE_NO', quoteNo);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REGISTRATION_NO', registrationNo);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'BUILD_DATE', buildDate);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'BODY_STYLE', bodyStyle);

        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'REPAIRER_TAX_INVOICE_NO', repairerTaxInvoiceNo);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'PAY_AUTHORISED_DATE', payAuthorisedDate);
        paymentAuthorisedXml = this.replaceXmlPlaceholder(paymentAuthorisedXml, 'PAYMENT_AMOUNT', paymentAmount);

        return paymentAuthorisedXml;
    }

    async getAndModifyQMLPaymentAuthorised(): Promise<string> {
        return await step('Read copied XML and create Payment Authorised XML', async () => {
            const xmlContent = await this.getXmlContent();
    ...
    async pasteModifiedQMLPaymentAuthorised(): Promise<void> {
        await step('Paste Payment Authorised XML into CodeMirror editor', async () => {
            if (!this.modifiedXml) {
                throw new Error(
    ...
    async clickInvoiceCheckBox(): Promise<void> {
        await step('Click Invoice checkbox', async () => {
            await this.invoiceCheckBox.first().locator('input[type="checkbox"]').click({ force: true });
        });
    }
    ...
    async validateQuoteStatus(expectedStatus: string): Promise<void> {
        await step(`Validate Quote Status is: ${expectedStatus}`, async () => {
            ...
            const selectedStatus = await this.quoteStatusSelect.locator('option:checked').innerText();
    ...
    async clickPaymentAuthorised(): Promise<void> {
        await step('Select Payment Authorised from dropdown', async () => {
            await this.messageTypeOption.selectOption({ label: 'Payment Authorised' });
        });
    }
    ...
    async selectViewMsgFirstRow(): Promise<void> {
        await step('Select First Row from the View Messages', async () => {
            ...
            const responsePromise = new Promise<{ uuid: string; messageData: string }>((resolve, reject) => {
                const handler = async (response: Response) => {
                    const url = response.url();
                    const match = url.match(/webservicetransactions\/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})$/);
                    if (match) {
            ...
            const { uuid, messageData } = await Promise.race([
                ...
                new Promise<never>((_, reject) =>
                    setTimeout(() => reject(new Error('Response capture timed out after 10s')), 10000)
                ),
            ]);
    ...
    async verifyQuotingNavButtonsDisabled(): Promise<void> {
        await step('Verify Manual Quote, Templates and Vehicle Sections buttons are disabled', async () => {
    ...
    async clickLoadMessageButton(expectedH1?: string, expectedQuoteStatus?: string): Promise<void> {
        if (expectedH1) {
            await step('Click Load Message, verify preview tab, close it, click Yes, switch to new tab, and verify quote status', async () => {
                ...
                await expect(previewTab.getByRole('heading', { name: expectedH1 })).toBeVisible();
    ...
    async clickContinueAndVerify(expectedH1: string): Promise<{ quoteTab: Page; quoteNumber: string }> {
        return await step('Click Continue, verify new tab, click Yes, and capture quote number', async () => {
            const [previewTab] = await Promise.all([
            ...
            const quoteNumberLocator = quoteTab.locator('.quote-info .has-text-weight-bold').first();
    ...
    async searchAndOpenQuoteByNumber(quoteNumber: string): Promise<void> {
        await step(`Search and open Quote No: ${quoteNumber}`, async () => {
            ...
            await Promise.all([
                this.page.waitForURL(/\/v2\/quotes\//, { waitUntil: 'domcontentloaded' }),
                quoteLink.click()
            ]);
        });
    }
    ...
    async selectingQuotingItemsRandom(): Promise<void> {
        ...
        if (totalPlusButtons < 5) {
            throw new Error(`Expected at least 5 plus buttons, but found ${totalPlusButtons}`);
        }
    ...
    async randomPriceForItems(): Promise<void> {
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            if (existingValue === '' || existingValue === '0' || existingValue === '0.00') {
    ...
    async handleInvoiceFlow(generatedQuoteNumber: string, subNavBarPage: any): Promise<void> {
        await step('Handle Invoice Flow', async () => {
            const createInvoiceVisible = await this.createInvoice.isVisible().catch(() => false);
            const invoiceCheckBoxVisible = await this.invoiceCheckBox.first()
            ...
            throw new Error('No valid invoice action found. Create Invoice or Raise Invoice option is not visible.');
        });
    }
    ...
    generateRandomPartValuesFromXml(xml: string): number[] {
        const partValueMatches = xml.match(/<PartValue>[\s\S]*?<\/PartValue>/g) || [];
    ...
    modifyQMLQuoteWithRandomChangedPartValues(xml: string): {
        ...
        updatedXml = updatedXml.replace(
            /<Status>(ADDED|ACCEPTED|REJECTED|UNCHANGED|CHANGED)<\/Status>/g,
            '<Status>CHANGED</Status>'
        );

        const grandTotalExTax = randomPartValues.reduce((sum, value) => sum + value, 0);
        const taxAmount = this.roundToTwo(grandTotalExTax * 0.10);
    ...
    async getAndModifyQMLQuoteWithRandomChangedPartValues(): Promise<{
        ...
        return await step('Read copied XML, randomize part values, update totals and tax', async () => {
            const xmlContent = await this.getXmlContent();

            this.extractedMessageNo = this.extractMessageNo(xmlContent);
            this.incrementedMessageNo = String(Number(this.extractedMessageNo) + 1);

            const result = this.modifyQMLQuoteWithRandomChangedPartValues(xmlContent);
